...
public class OAuthSAMLClient {
	...
	public OAuthSAMLClient (String host, String user){
		this.host = host;
		this.user = user;
		this.entityId = host.substring(host.indexOf("https://") + "https://".length());
		this.tokenServiceUrl = this.host + "/sap/bc/sec/oauth2/token";
	}
	...
	public String generateSAMLAssetion() throws IOException {
		...
		samlAssertion = samlAssertion.replaceAll("@NAME_ID", this.user);
		...
		samlAssertion = samlAssertion.replaceAll("@SP_ENTITY_ID", this.entityId);
		samlAssertion = samlAssertion.replaceAll("@TOKEN_SERVICE_URL",
				this.tokenServiceUrl);

		try {
			String filePath = OAuthSAMLClient.class.getResource("mykeystore.jks")
					.getPath();
